...
ALTER TABLE activities ADD COLUMN IF NOT EXISTS archived_by_type VARCHAR(20);  -- 'pembina' | 'admin'
...
ALTER TABLE activities ADD COLUMN IF NOT EXISTS cancellation_reason TEXT;  -- untuk force-cancel
...
CREATE TABLE IF NOT EXISTS quest_audit_logs (
  ...
  quest_id UUID NOT NULL,  -- Tidak pakai FK karena untuk delete permanen, quest_id tetap ada di log
  quest_title TEXT,  -- Snapshot title saat aksi (kalau quest sudah didelete permanen)
  ...
  changes JSONB,  -- Before/after diff (untuk edit); atau null untuk delete/archive/restore
  affected_intern_id UUID,  -- Untuk force_cancel: intern mana yang dibatalkan
  reason TEXT,
