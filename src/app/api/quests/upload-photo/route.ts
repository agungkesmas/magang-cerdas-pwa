// ============================================================
// /api/quests/upload-photo — Upload foto bukti saat submit quest
//
...
export async function POST(req: NextRequest) {
  try {
    ...
    const fileName = `${intern.intern_id}/quest-${uuidv4()}.${ext}`;

    const { data, error } = await supabase.storage
      .from('attendance-photos')
      .upload(`quest-photos/${fileName}`, file, { contentType: file.type, upsert: false });
