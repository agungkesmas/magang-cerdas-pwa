// ============================================================
// /api/pembina/batch-password — Admin: ubah password massal pembina magang
//
// Body: { ids: string[] (min 1), mode: 'auto' | 'custom', custom_password?: string }
//   - mode='auto': generate password baru per pembina (format Pembina2026!xxxx)
//   - mode='custom': pakai 1 password untuk semua pembina yang dipilih
//
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: pems, error: fErr } = await supabase
      .from('pembina_magang')
    ...
    if (!pems || pems.length === 0) {
      return NextResponse.json({ error: 'Tidak ada pembina ditemukan untuk ID yang dipilih.' }, { status: 404 });
    }
    ...
    for (const p of pems) {
      ...
      try {
        ...
        const { error: uErr } = await supabase
          ...
          .eq('id', p.id);
        if (uErr) {
          errors.push({ id: p.id, name: p.name, error: uErr.message });
        } else {
          results.push({ id: p.id, name: p.name, email: p.email, raw_password: rawPassword });
        }
      } catch (e: any) {
