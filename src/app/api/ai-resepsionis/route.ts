...
6. **Pembina Magang** — kelola akun pembina (staff BPJTK). Tambah pembina: isi nama, email, departemen (Pelayanan/Pemasaran/Keuangan/Lintas Bidang), telepon. Auto-generate ID PB-XXXX + password. **Auto-link ke grup departemen aktif** yang sama (skip 'Lintas Bidang'). Bisa reset password, toggle aktif, hapus. **Tombol Print Kartu Kredensial** per pembina (icon printer di sebelah tombol Copy) — untuk cetak kartu login PB-XXXX + password + login URL, siap disebarkan ke pembina.
