...
7. **Sertifikat** — terbitkan sertifikat. Tier otomatis berdasarkan EXP: Excellence (≥1000 EXP) 🏆, Competent (≥500 EXP) ✅, Participation (<500 EXP) 📋. Threshold hardcoded 500 & 1000. Wajib ada Kepala Cabang aktif (set di Pengaturan → Kepala Cabang). Tombol "Terbitkan" generate verification_id. Setelah terbitkan, sertifikat muncul di riwayat aktivitas peserta (menu **Riwayat Aktivitas Peserta**).
...
function stubAnswer(dashboard: string, question: string): string {
  ...
  if (dashboard === 'admin') {
    ...
    if (q.includes('grup') || (q.includes('chat') && !q.includes('clean'))) return 'Untuk kelola grup: menu **Kelola Grup** (buat grup, tambah anggota, arsipkan, hapus). Untuk kirim pengumuman/broadcast ke peserta: menu **Chat Grup** → pilih grup sistem (mis. **All Peserta Magang** untuk broadcast ke semua, atau **Magang - Pemasaran** untuk departemen) → kirim text/foto/dokumen WhatsApp-style. Peserta terima realtime di chat grup mereka.';
    ...
    return 'Maaf, saya hanya melayani pertanyaan seputar menu di dashboard ini. Coba tanya tentang: Peserta Magang, Permintaan Magang, Riwayat Aktivitas Peserta, Kehadiran, Chat Grup (broadcast/pengumuman), Kelola Grup, Sertifikat, Pembina, Institusi & BKK, atau Pengaturan (Kantor/AI Provider/Keamanan & Data/Kepala Cabang).';
  }
