...
export default function SchoolDetailPage() {
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [schoolRes, internsRes] = await Promise.all([
        fetch(`/api/schools`),
      ...
      if (internsData.success) {
        setInterns(internsData.interns.filter((i: Intern) => i.school_origin === school?.name));
      }
      ...
      if (tData.success) {
        ...
        // Also re-filter interns now that school is loaded
        if (schoolData.success) {
          const found = schoolData.schools.find((s: School) => s.id === schoolId);
          if (found) {
            setInterns(internsData.interns.filter((i: Intern) => i.school_origin === found.name));
          }
        }
      }
    } finally {
...
Selamat membimbing siswa magang di BPJS Ketenagakerjaan Cabang Cirebon!`;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gradient-to-br from-bpjs-green to-bpjs-green-dark rounded-2xl p-6 text-white shadow-lg">
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          ...
          <div className="bg-white/10 rounded-lg p-3">
            ...
            <div className="text-xs text-white/70">Magang Aktif</div>
          </div>
          <div className="bg-white/10 rounded-lg p-3">
            <div className="text-2xl font-bold">{interns.filter((i) => i.is_active).length}</div>
            <div className="text-xs text-white/70">Total Magang</div>
          </div>
        </div>
      </div>
      ...
      {/* Interns from this school */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center gap-2 mb-4">
          <User className="w-5 h-5 text-bpjs-blue" />
          <h2 className="font-bold text-gray-900" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
            Magang dari {school.name} ({interns.length})
          </h2>
        </div>
        {interns.length === 0 ? (
          <div className="text-center py-6 text-gray-500 text-sm">
            Belum ada magang dari sekolah ini.
          </div>
        ) : (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-500">
                    {i.major} • {i.department} • {i.total_exp} EXP
                  </p>
                </div>
                {!i.logbook_enabled && (
